...
class GameClass:
    def __init__(self, draw_screen, fps):
        ...
        # if True, obstacles are randomly spanned in the space
    ...
    def check_reach_goal(self):
        distance2goal = self.robot_body.position.get_distance(
            self.goal.position)
    ...
    def second_step(self, action):
        ...
        for event in pygame.event.get():
            if event.type == KEYDOWN and event.key == K_RIGHT:
                self.robot_body.velocity = self.robot_body.velocity.rotated_degrees(
                    -45)
